...
from reporter.database_manager import (
    ...
    get_all_plans, add_transaction, get_all_activity_for_member,
...
def test_get_all_members_multiple(db_conn):
    ...
    for i, expected_name in enumerate(expected_names_sorted):
        ...
        assert members[i][3] is None # Join date should be None if no activity
...
def test_add_group_class_transaction(db_conn):
    ...
    # 4. Call add_transaction for Group Class
    ...
    cursor.execute(
        ...
        "FROM transactions WHERE member_id = ? AND transaction_type = ?",
        (member_id, 'Group Class')
    )
    gc_record = cursor.fetchone()
    assert gc_record is not None, "Group Class transaction record not found."

    (gc_member_id, gc_plan_id, gc_payment_date, gc_start_date,
     gc_end_date, gc_amount_paid, gc_payment_method, gc_transaction_type) = gc_record

    assert gc_member_id == member_id
    assert gc_plan_id == plan_id
    assert gc_payment_date == payment_date_str
    assert gc_start_date == start_date_str
    assert gc_amount_paid == amount_paid
    assert gc_payment_method == payment_method
    assert gc_transaction_type == 'Group Class'
    ...
    assert gc_end_date == expected_end_date_str, \
        f"End date mismatch. Expected {expected_end_date_str}, got {gc_end_date}."
...
def test_add_personal_training_transaction(db_conn):
    ...
    assert success is True, "add_transaction for Personal Training should return True on success."

    # 4. Verify directly in the database
    cursor.execute(
        "SELECT member_id, start_date, sessions, amount_paid, transaction_type, plan_id "
        "FROM transactions WHERE member_id = ? AND transaction_type = ?",
        (member_id, 'Personal Training')
    )
    pt_record = cursor.fetchone()
    assert pt_record is not None, "Personal Training transaction record not found."

    (db_member_id, db_start_date, db_sessions, db_amount_paid, db_transaction_type, db_plan_id) = pt_record
    ...
    assert db_plan_id is None, "plan_id should be None for Personal Training transactions."
...
def test_get_finance_report_with_pt_bookings(db_conn):
    ...
    assert add_transaction(transaction_type='Personal Training', member_id=member_f1_id, start_date=target_month_date2, payment_date=target_month_date2, sessions=10, amount_paid=200.00) # 200.00
    assert add_transaction(transaction_type='Personal Training', member_id=member_f2_id, start_date=target_month_date1, payment_date=target_month_date1, sessions=5, amount_paid=150.00)  # 150.00
    # Outside target month
    assert add_transaction(transaction_type='Personal Training', member_id=member_f1_id, start_date=other_month_date, payment_date=other_month_date, sessions=8, amount_paid=180.00)
...
def test_join_date_standardization_new_member_then_pt_booking(db_conn):
    ...
    assert add_transaction(transaction_type='Personal Training', member_id=member_id, start_date=pt_start_date, payment_date=pt_start_date, sessions=10, amount_paid=300)
...
def test_join_date_standardization_existing_member_earlier_activity(db_conn):
    ...
    assert add_transaction(transaction_type='Personal Training', member_id=member_id, start_date=initial_activity_date, payment_date=initial_activity_date, sessions=5, amount_paid=250) # This will set join_date to 2023-03-15
...
def test_join_date_standardization_existing_member_later_activity(db_conn):
    ...
    assert add_transaction(transaction_type='Personal Training', member_id=member_id, start_date=later_pt_start_date, payment_date=later_pt_start_date, sessions=8, amount_paid=280)
